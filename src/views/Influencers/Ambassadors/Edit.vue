        ...
        <div class="container-fluid d-content">
            <div class="title-content mb-3">
                ...
                <h3>Edit campaign <b class="text-primary">{{ item.name }}</b>, the current campaign is <span
                    class="badge bg-primary">{{ item.status }}</span></h3>
                <a class="mt-2 cursor-pointer" @click="$router.push({name: 'ambassadors'})">Back</a>
            </div>

            <!-- Add parent -->
            <add-influencer-in-campaign :agencies="agencies" @add="addChildren"/>
...
export default {
    ...
    methods: {
        async getItem() {
            ...
            const {data} = await AmbassadorService.get(this.id)
                .catch(e => {
        ...
        async getAgencies() {
            ...
            const {data} = await AmbassadorService.agencies(this.id)
                .catch(e => {
                    this.$root.$emit('gcLoading', 0);
                    this.$toast.error(e);
                });
        ...
        async addChildren(e) {
            ...
            const res = await AmbassadorService.addInfluencer(params)
                .catch(e => {
                    this.$root.$emit('gcLoading', 0);
